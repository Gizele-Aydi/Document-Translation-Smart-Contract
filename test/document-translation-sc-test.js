...
describe('DocumentTranslationContract - Full Functionality', () => {
    ...
    beforeEach(() => {
        ...
        ctx.stub = {
            ...
            getHistoryForKey: async () => {
                // Mock history - return empty array for now
                return {
            ...
            getStateByRange: async () => {
                ...
                const allValues = Object.keys(ledger).map(k => ({ 
                    key: k, 
                    value: Buffer.from(ledger[k]) 
                }));
                return {
                    async *[Symbol.asyncIterator]() {
    ...
    it('should register a new document', async () => {
        ...
        expect(doc.ownerID).to.equal('CITIZEN1');
    });
    ...
    it('should assign translator to document', async () => {
        ...
        expect(assigned.status).to.equal('IN_TRANSLATION');
    });
    ...
    it('should submit a translation', async () => {
        ...
        expect(translation.verificationStatus).to.equal('PENDING_QA');

        const doc = await contract.getDocument(ctx, 'DOC102');
        expect(doc.translations).to.have.lengthOf(1);
        expect(doc.status).to.equal('TRANSLATED');
    });
    ...
    it('should QA reject a translation and allow resubmission', async () => {
        ...
        await contract.submitTranslation(ctx, 'DOC104', 'TRANS103', 'English');
    ...
    it('should retrieve document history', async () => {
    ...
    it('should throw error when document does not exist', async () => {
        try {
            await contract.getDocument(ctx, 'NONEXISTENT');
